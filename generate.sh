...
if [[ $# -eq 0 ]] || [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
    ...
    echo ""
    echo "Usage:"
    echo "  ./generate.sh \"your prompt\" [options]        # Standard SD 2.1"
    echo "  ./generate.sh --xl \"your prompt\" [options]   # SDXL (higher quality)"
    echo ""
    echo "Options:"
    ...
    echo "  ./generate.sh --xl \"photorealistic portrait\" -s 40"
    exit 0
fi
...
if [[ "$USE_SDXL" == true ]]; then
    echo "Using SDXL (high quality mode)..."
    docker compose run --rm sdxl "$@" -o "/app/output/$(basename "${2:-output_xl.png}")"
else
    echo "Using Stable Diffusion 2.1..."
